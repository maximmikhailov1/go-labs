...
//		Premade: true,
//	}
//	var student models2.User
//	result := database.DB.First(&student, studentID)
//	if result.Error != nil {
//		return result.Error
//	}
...
//		return result.Error
//	}
//	appRes := database.DB.Model(&team).Association("Members").Append(&student)
//	if appRes != nil {
//		return appRes
//	}
//	log.Info(fiber.Map{
...
//	}
//	var team models2.Team
//	err := database.DB.Find(&team, "code = ?", teamCode).Error
//	if err != nil {
//		return c.Status(http.StatusBadRequest).JSON("team not found")
//	}
//	err = database.DB.Model(&user).Association("Teams").Delete(&team)
//	if err != nil {
...
//	}
//	var teamWanted models2.Team
//	result = database.DB.Where("code = ?", teamCode).First(&teamWanted)
//	if result.Error != nil {
//		return c.Status(http.StatusBadRequest).JSON("no team under this code")
//	}
//
//	membersCount := database.DB.Model(&teamWanted).Association("Members").Count()
//	if membersCount <= 4 {
//		return c.Status(http.StatusBadRequest).JSON("cant join a full team")
//	}
//
//	err := database.DB.Model(&teamWanted).Association("Members").Append(&student)
//	if err != nil {
...
//	var team models2.Team // team.code - UNIQUE поэтому не боимся не использовать слайс
//	database.DB.Model(&student).Where("code = ?", teamCode).Association("Teams").Find(&team)
//	if team.Code == "" {
//		return c.Status(http.StatusBadRequest).JSON("команда не существует или вы в ней не состоите")
//	}
//	// initializers.DB.Model(&team).Association("Teams").Find(&student)
//	database.DB.Model(&team).Update("name", teamNewName)
//	return c.Status(http.StatusOK).JSON("successful")
//}
...
//	if result.Error != nil {
//		return c.Status(http.StatusUnauthorized).JSON("failed to get team")
//	}
//
...
func LabsFirstBySubject(c *fiber.Ctx) error {
	...
	var labs []models2.Lab
	err := database.DB.Where("subject_id = ?", uint(subjectID)).Find(&labs).Error
	if err != nil {
...
func UserLabsIndex(c *fiber.Ctx) error {
	...
	if studentRole != "student" {
		var emptyListForTutors = models2.Lab{
			ID:                      0,
			SwitchesRequired:        0,
			RoutersRequired:         0,
		...
		return c.Status(http.StatusOK).JSON([]models2.Lab{emptyListForTutors})
	}
	if studentGroup == "" {
	...
	if err := database.DB.Preload("Subject.Labs").First(&group, "name = ?", studentGroup).Error; err != nil {
	...
	if group.SubjectID == nil {
		return c.Status(http.StatusOK).JSON([]models2.Lab{})
	}

	// Получаем лабораторные работы по предмету группы
	var labs []models2.Lab
	if err := database.DB.Where("subject_id = ?", group.SubjectID).Find(&labs).Error; err != nil {
		return c.Status(http.StatusInternalServerError).JSON(fiber.Map{
...
//
//			`).
//			Joins("JOIN team ON entries.team_id = team.id").
//			Joins("JOIN users_teams ON team.id = users_teams.team_id").
...
//			Group("entries.id, labs.number, records.lab_date, records.class_number, labs.description,records.audience_number, entries.status, team.name").
...
//
//		var records []models2.Record
//		result := database.DB.Debug().
...
//	var existingEntries []models2.Entry
...
//
//	err = database.DB.Preload("Tutor", func(db *gorm.DB) *gorm.DB {
...
func GroupsIndex(c *fiber.Ctx) error {
	...
	err := database.DB.Debug().Preload("Subject", func(db *gorm.DB) *gorm.DB {
...
func GroupUpdateSubject(c *fiber.Ctx) error {
	...
	log.Info(subjectID)
	log.Info(groupID)
	var subject models2.Subject
	var group models2.Group
	if err := database.DB.Debug().First(&subject, uint(subjectID)).Error; err != nil {
		return c.Status(http.StatusBadRequest).JSON("failed to find subject with id provided")
	}
	if err := database.DB.Debug().First(&group, uint(groupID)).Error; err != nil {
		return c.Status(http.StatusBadRequest).JSON("failed to find a group with id provided")
	}
	log.Info(subject)
	log.Info(group)
	if err := database.DB.Debug().Model(&subject).Association("Groups").Append(&group); err != nil {
...
//			if err := tx.Model(&entry.Team).Association("Members").Delete(&models2.User{ID: userID}); err != nil {
...
//	}
//	var entry models2.Entry
//	if err := database.DB.Preload("Team").First(&entry, requestBody.EntryID).Error; err != nil {
...
//	var student models2.User
//	if err := database.DB.First(&student, requestBody.StudentID).Error; err != nil {
...
//	if err := database.DB.Model(&entry.Team).Association("Members").Delete(&student); err != nil {
...
//	if err := database.DB.Model(&entry).Update("updated_at", time.Now()).Error; err != nil {
