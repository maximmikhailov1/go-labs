...
type Record struct {
	ID uint
	gorm.Model
	LabDate                  datatypes.Date `gorm:"index:idx_record_main;not null"`
	ClassNumber              int            `gorm:"index:idx_record_main;check:class_number > 0 AND class_number < 9"`
	AudienceNumber           int            `gorm:"index:idx_record_main;not null"`
	AudienceID               *uint
	Audience                 *Audience `gorm:"foreignKey:AudienceID"`
